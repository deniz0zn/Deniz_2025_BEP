...
def process_logs(path: str, delta_name: str, delta_stats_list: list):
    ...
    for _,event in event_log.iterrows():
        ...
        if cases.get(case_id) is None:
            cases[case_id] = Case(event, delta_name,delta)
            # print(f"[PROCESS LOGS] New Case added: {case_id}")

        else:
            cases[case_id].update(event, delta)

        # delta.process_case_status(cases[case_id])

    # delta.process_event(event, cases)
    ...
    for id, obj in cases.items():
        obj.update_time(current_T, delta_name)
        delta.process_case_status(obj)

    delta_stats_list.append(delta.generate_report())
...
if __name__ == '__main__':
    ...
    process_logs(initial_log_path, delta_name="initial_log", delta_stats_list=delta_stats_list)

    # Process all delta logs
    for file, delta_name in delta_logs:
        # print(f"[MAIN] Processing the delta log file: {file}")
        process_logs(file, delta_name, delta_stats_list)

    delta_df = pd.DataFrame(delta_stats_list)
    ...
    case_dict = {case_id: vars(case_obj) for case_id, case_obj in cases.items()}
    ...
    print(f"Number of Cases that is processed: {size}")
    print(f"Number of Complete Cases: {len(complete_cases)}\nRatio: {len(complete_cases)/len(case_df)}\n")

    finished_cases = case_df[case_df["finished"]]
    print(f"Ratio of Complete cases/Finished Cases: {len(complete_cases)/len(finished_cases)}\n")
